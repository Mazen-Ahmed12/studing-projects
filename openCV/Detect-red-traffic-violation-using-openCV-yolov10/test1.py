...
def process_frame(frame):
    ...
    red_mask = cv2.inRange(hsv, lower_range1, upper_range1)
    ...
    for c in cnts:
        if cv2.contourArea(c) > 50:
            ...
            # Determine the color of the contour
                if cv2.countNonZero(green_mask[y:y+h, x:x+w]) > 0:  # Green color range
                   color = (0, 255, 0)  # Green color for the rectangle
                   text_color = (0, 255, 0)  # Green text
                   label = "GREEN"
                elif cv2.countNonZero(red_mask[y:y+h, x:x+w]) > 0:  # Red color range
